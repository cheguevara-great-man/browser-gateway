...
当前版本为 **0.4.0**。服务器链路支持 HTTP/2 连接复用、账号认证、可信 TLS 证书、目标访问限制、健康检查和证书自动续期。
...
|---|---|---|
| **规则模式**（新安装默认） | 常用国内网站、本地地址和内网地址直连；其余网站使用 Gateway | 仅未命中的境外/其他网站 |
| **全局模式** | 除本地/内网地址外，全部使用 Gateway | 是 |
| **直连模式** | Chrome 由扩展强制使用 `DIRECT`，不使用 Windows 系统代理 | 否 |

规则模式的国内判断在 Chrome 本地执行：覆盖 `.cn`、政务/教育域名及百度、腾讯、阿里、京东、网易、哔哩哔哩、知乎、抖音等常用国内服务。它不是可自动更新的完整中国域名数据库；未列入规则的国内 `.com` 网站仍会走服务器。需要完全按 IP/GeoIP 分流时，应使用具备规则集能力的本地客户端；本扩展刻意不安装或启动这类客户端。

...
$installer = Join-Path $root 'server\install.sh'
...
& $scp @common $installer "root@${Server}:/root/browser-gateway-install.sh"
if ($LASTEXITCODE -ne 0) { throw 'Failed to upload the server installer.' }
& $ssh @common "root@$Server" "chmod 0700 /root/browser-gateway-install.sh && /root/browser-gateway-install.sh '$Server'"
...
Write-Host 'Browser Gateway server deployed successfully.' -ForegroundColor Green
